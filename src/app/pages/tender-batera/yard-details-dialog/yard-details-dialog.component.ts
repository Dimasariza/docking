...
export class YardDetailsDialog implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.tableData = {
            ...
            no_kontrak, 
            yard_total_quote : 
            this.tableFunction.convertToCurrency(mata_uang, parseInt(yard_total_quote)) ,
            sum_internal_adjusment : 
            this.tableFunction.convertToCurrency(mata_uang, parseInt(sum_internal_adjusment)),
            general_diskon_persen :
            `
            ( ${general_diskon_persen} %)
            ${this.tableFunction.convertToCurrency(mata_uang, (
                parseInt(general_diskon_persen) * ownerTotalCost
            ) / 100 )}
            `
            ,
            additional_diskon :
            this.tableFunction.convertToCurrency(mata_uang, additional_diskon),
            mata_uang,
            off_hire_period : proyek 
            ? `${off_hire_period} Days` 
            : null,
            off_hire_rate_per_day :
            this.tableFunction.convertToCurrency(mata_uang, off_hire_rate_per_day),
            off_hire_bunker_per_day :
            this.tableFunction.convertToCurrency(mata_uang, off_hire_bunker_per_day),
            ownerTotalCost :
            this.tableFunction.convertToCurrency(mata_uang, ownerTotalCost),
        }
    }
