...
export class YardDialogComponent implements OnInit{
    ...
    ngOnInit(): void {
        this.profileService.getAllUsers({ role : 'shipyard', status : 'active'})
            .subscribe(({data} : any) => {
    ...
    closeDialog (arr = null) {
        ...
        yard_total_quote = this.commonFunction.priceAmount(yard_total_quote)
        additional_diskon = this.commonFunction.priceAmount(additional_diskon)
        sum_internal_adjusment = this.commonFunction.priceAmount(sum_internal_adjusment)
