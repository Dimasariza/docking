...
export class YardDetailsDialog implements OnInit {
    ...
    ngOnInit(): void {
        ...
        const ownerTotalCost 
        = this.commonFunction.priceAmount(off_hire_rate_per_day) 
        + this.commonFunction.priceAmount(off_hire_bunker_per_day);
        ...
        this.tableData = {
            ...
            no_kontrak, 
            yard_total_quote : 
            this.commonFunction.convertToCurrency(mata_uang, yard_total_quote),
            sum_internal_adjusment : 
            this.commonFunction.convertToCurrency(mata_uang, sum_internal_adjusment),
            general_diskon_persen :
            `
            ( ${general_diskon_persen} %)
