...
export class ProjectBateraComponent implements OnInit, OnDestroy {
  ...
  tableHead = [ 
    ...
    { type : 'drop-down', placeholder : 'Status', option : ['All', ...this.tableFunction.jobStatus], target : 'sort' },
  ...
  generateTableDatas(datas) {
    this.projectDatas = datas.map(data => {
      ...
      const phaseStatus = this.tableFunction.setPhase(phase);
      ...
      if(this.tableFunction.arrayNotEmpty(data.work_area))
  ...
  sortTableByFilter({column, value} : any) {
    ...
    this.dataTable = this.tableFunction.populateData(sortingData, false) 
  ...
  onUploadData(data, title) {
    ...
    if(title == 'Add Project') {
      ...
      const off_hire_start = this.tableFunction.transformDate(offHirePeriod.start);
      const off_hire_end = this.tableFunction.transformDate(offHirePeriod.end);
      const repair_start = this.tableFunction.transformDate(repairInDock.start);
      const repair_end = this.tableFunction.transformDate(repairInDock.end);
      const repair_in_dock_start = this.tableFunction.transformDate(repairPeriod.start);
      const repair_in_dock_end = this.tableFunction.transformDate(repairPeriod.end);
